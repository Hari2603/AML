...
class buyoldcar(generic.ListView):
    ...
    def get_queryset(self):
        qs=sellcar.objects.all().exclude(user_id=self.request.user.id)#.exclude(Email=request.user.email)
        sellcar_filtered_list = usedcarfilter(self.request.GET, queryset=qs)
        return sellcar_filtered_list.qs

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        myFilter=usedcarfilter(self.request.GET,queryset=self.get_queryset())
...
class details(generic.DetailView):
    model=sellcar
    template_name='accounts/sellcar_detail.html'
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        self.request.session['selectedcar'] = self.kwargs.get('pk')
        #print(self.request.session['sellcar'])
        return context
...
def used_view(request,pk): 
    # dictionary for initial data with      
    # field names as keys 
    context ={} 
    
    sellcar_list=sellcar.objects.filter(id=pk).first()
    
    # add the dictionary during initialization 
    context={"sellcar":sellcar_list}
          
    return render(request, "accounts/particularcar.html", context) 
...
class myads(generic.ListView):
    model=sellcar
    template_name='accounts/myads_list.html'
    def get_queryset(self):
        return sellcar.objects.filter(id__in=sellcar.objects.filter(user=self.request.user))
        #return favv.objects.filter(User=self.request.user).values_list('car', flat=True)


def delete(request):
    sellcar.objects.filter(id__in=sellcar.objects.filter(user=request.user)).delete()
